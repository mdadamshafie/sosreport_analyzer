"""
SOSreport Analyzer V2 - Streamlit Web Application
...
Includes Critical Events Detection for system issues.
"""
...
LOG_PATTERNS = {
    ...
    "CPU & Kernel Panic": [
        ...
        'MCE: Machine Check', 'procs_blocked'
    ],
    "Security & Antivirus": [
...
st.set_page_config(
    page_title="SOSreport Analyzer V2",
    page_icon="ðŸ“Š",
    layout="wide",
                    ...
                    for line in lines[1:]:  # Skip header
                        ...
                        if len(parts) >= 6:
                            ...
                            try:
                                ...
                                if any(fs in filesystem for fs in ['tmpfs', 'devtmpfs', 'overlay']):
                                    continue
...
def detect_installed_packages(sosreport_path: str) -> dict:
    ...
    rpm_files = [
        os.path.join(sosreport_path, "sos_commands", "rpm", "rpm_-qa"),
        os.path.join(sosreport_path, "sos_commands", "rpm", "rpm_-qa_--qf_..."),
        os.path.join(sosreport_path, "installed-rpms"),
    ]
                ...
                with open(rpm_file, 'r') as f:
                    for line in f:
                        ...
                        if 'rhui' in pkg_lower:
                            packages['rhui'].append(pkg)
                        elif pkg_lower.startswith('kernel-'):
                            packages['kernel'].append(pkg)
...
def push_sar_to_influxdb(metrics: List[dict], hostname: str, progress_callback=None) -> Tuple[int, str]:
    ...
    # Show date range of SAR metrics being pushed
    timestamps = [m['timestamp'] for m in metrics if m.get('timestamp')]
    if timestamps:
        min_ts = min(timestamps)
        max_ts = max(timestamps)
        st.write(f"**ðŸ“… SAR metrics date range being pushed:** {min_ts.strftime('%Y-%m-%d %H:%M')} to {max_ts.strftime('%Y-%m-%d %H:%M')}")
        
        # Show metrics count by date
        metrics_by_date = {}
        for ts in timestamps:
            date_str = ts.strftime('%Y-%m-%d')
            metrics_by_date[date_str] = metrics_by_date.get(date_str, 0) + 1
        st.write(f"**Dates:** {', '.join(f'{d}: {c}' for d, c in sorted(metrics_by_date.items()))}")
    ...
    for i in range(0, len(metrics), batch_size):
        ...
        if lines:
            # Debug: Show sample lines being pushed (first batch only)
            if i == 0:
                st.write("**ðŸ” Debug: Sample InfluxDB lines (first 3):**")
                for sample_line in lines[:3]:
                    st.code(sample_line, language="text")
...
def push_logs_to_loki(logs: List[dict], hostname: str, progress_callback=None) -> Tuple[int, str]:
    ...
    # Show date range of logs being pushed
    timestamps = [log['timestamp'] for log in logs if log.get('timestamp')]
    if timestamps:
        min_ts = min(timestamps)
        max_ts = max(timestamps)
        st.write(f"**ðŸ“… Logs date range being pushed:** {min_ts.strftime('%Y-%m-%d %H:%M')} to {max_ts.strftime('%Y-%m-%d %H:%M')}")
    ...
    for source, values in streams.items():
        ...
        # Debug: Show first and last timestamps for this source
        if values:
            first_ts = datetime.fromtimestamp(int(values[0][0]) / 1e9)
            last_ts = datetime.fromtimestamp(int(values[-1][0]) / 1e9)
            st.write(f"**{source}:** {len(values):,} entries from {first_ts.strftime('%Y-%m-%d')} to {last_ts.strftime('%Y-%m-%d')}")
        
        # Debug: Show sample timestamps for first source
        if not first_batch_shown and values:
            st.write("**ðŸ” Debug: Sample Loki timestamps (first 3):**")
            for ts_ns, msg in values[:3]:
                # Convert nanoseconds back to datetime for display
                ts_sec = int(ts_ns) / 1e9
                ts_dt = datetime.fromtimestamp(ts_sec)
                st.code(f"{ts_dt.strftime('%Y-%m-%d %H:%M:%S')} - {msg[:80]}...", language="text")
        ...
        for i in range(0, len(values), batch_size):
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    errors.append(f"{source} batch {i//batch_size}: HTTP {response.status_code} - {error_text}")
            except Exception as e:
                errors.append(f"{source} batch {i//batch_size}: Exception - {str(e)}")
...
def create_grafana_dashboard(hostname: str, time_from: datetime = None, time_to: datetime = None, 
    ...
    # Build system overview markdown
    sys_md = "## System Overview\\n\\n"
    if system_info:
        cpu_info = system_info.get('cpu_info', {})
        mem_info = system_info.get('memory_info', {})
        
        sys_md += f"| **Property** | **Value** |\\n"
        sys_md += f"|:---|:---|\\n"
        sys_md += f"| Hostname | `{hostname}` |\\n"
        sys_md += f"| OS | `{system_info.get('os_release', 'N/A')}` |\\n"
        sys_md += f"| Kernel | `{system_info.get('kernel', 'N/A')}` |\\n"
        sys_md += f"| CPUs | `{cpu_info.get('cores', 'N/A')}` |\\n"
        sys_md += f"| CPU Model | `{cpu_info.get('model', 'N/A')[:50]}` |\\n"
        sys_md += f"| Memory | `{mem_info.get('total_gb', 'N/A')}` |\\n"
        sys_md += f"| Architecture | `{cpu_info.get('architecture', 'N/A')}` |\\n"
        sys_md += f"| SELinux | `{system_info.get('selinux', 'N/A')}` |\\n"
        sys_md += f"| Uptime | `{system_info.get('uptime', 'N/A')}` |\\n"
        sys_md += f"| SOSreport Date | `{system_info.get('date', 'N/A')}` |\\n"
    else:
        sys_md += "*System information not available*\\n"
    
    # Build anomalies markdown
    anomaly_md = "## Performance Peaks\\n\\n"
    if sar_anomalies:
        anomaly_md += "| **Metric** | **Peak** | **Average** | **When** |\\n"
        anomaly_md += "|:---|:---|:---|:---|\\n"
        
        if sar_anomalies['cpu']['samples'] > 0:
            cpu_time = sar_anomalies['cpu']['max_time']
            time_str = cpu_time.strftime('%m-%d %H:%M') if cpu_time else 'N/A'
            anomaly_md += f"| CPU Usage | `{sar_anomalies['cpu']['max_usage']}%` | `{sar_anomalies['cpu']['avg_usage']}%` | {time_str} |\\n"
        
        if sar_anomalies['memory']['samples'] > 0:
            mem_time = sar_anomalies['memory']['max_time']
            time_str = mem_time.strftime('%m-%d %H:%M') if mem_time else 'N/A'
            anomaly_md += f"| Memory | `{sar_anomalies['memory']['max_usage']}%` | `{sar_anomalies['memory']['avg_usage']}%` | {time_str} |\\n"
        
        if sar_anomalies['disk']['samples'] > 0:
            disk_time = sar_anomalies['disk']['max_time']
            time_str = disk_time.strftime('%m-%d %H:%M') if disk_time else 'N/A'
            anomaly_md += f"| Disk ({sar_anomalies['disk']['max_device']}) | `{sar_anomalies['disk']['max_util']}%` | - | {time_str} |\\n"
        
        if sar_anomalies['load']['samples'] > 0:
            load_time = sar_anomalies['load']['max_time']
            time_str = load_time.strftime('%m-%d %H:%M') if load_time else 'N/A'
            anomaly_md += f"| Load Avg | `{sar_anomalies['load']['max_load1']}/{sar_anomalies['load']['max_load5']}/{sar_anomalies['load']['max_load15']}` | - | {time_str} |\\n"
        
        if sar_anomalies['network']['samples'] > 0:
            net_time = sar_anomalies['network']['max_time']
            time_str = net_time.strftime('%m-%d %H:%M') if net_time else 'N/A'
            anomaly_md += f"| Network ({sar_anomalies['network']['max_interface']}) | RX:`{sar_anomalies['network']['max_rx']:.1f}` TX:`{sar_anomalies['network']['max_tx']:.1f}` KB/s | - | {time_str} |\\n"
    else:
        anomaly_md += "*Anomaly data not available*\\n"
    
    # System Overview Panel (Text/Markdown)
    panels.append({
        "gridPos": {"h": 10, "w": 12, "x": 0, "y": y_pos},
        "id": panel_id,
        "options": {
            "content": sys_md,
            "mode": "markdown"
    ...
    # Anomalies Panel (Text/Markdown)
    panels.append({
        "gridPos": {"h": 10, "w": 12, "x": 12, "y": y_pos},
        "id": panel_id,
        "options": {
            "content": anomaly_md,
            "mode": "markdown"
        },
        "title": "ðŸ“ˆ Performance Peaks & Anomalies",
        "type": "text"
    })
    panel_id += 1
    y_pos += 10
    ...
    # Row Header: Critical Events
    panels.append({
        "gridPos": {"h": 1, "w": 24, "x": 0, "y": y_pos},
        "id": panel_id,
        "title": "ðŸš¨ Critical Events Detection",
        "type": "row",
        "collapsed": False
    })
    panel_id += 1
    ...
    # Critical Events - File System & Disk
    panels.append({
        "datasource": {"type": "loki", "uid": loki_uid},
        "gridPos": {"h": 6, "w": 12, "x": 0, "y": y_pos},
        "id": panel_id,
        "options": {"dedupStrategy": "none", "enableLogDetails": True, "showTime": True, "sortOrder": "Ascending", "wrapLogMessage": True},
        "targets": [{"datasource": {"type": "loki", "uid": loki_uid},
            "expr": f'{{host="{hostname}", critical="true", category="File System & Disk"}}',
            "maxLines": 500,
            "queryType": "range",
            "refId": "A"}],
        "title": "ðŸ’¾ File System & Disk Issues",
        "type": "logs"
    })
    panel_id += 1
    
    # Critical Events - Memory/OOM
    panels.append({
        "datasource": {"type": "loki", "uid": loki_uid},
        "gridPos": {"h": 6, "w": 12, "x": 12, "y": y_pos},
        "id": panel_id,
        "options": {"dedupStrategy": "none", "enableLogDetails": True, "showTime": True, "sortOrder": "Ascending", "wrapLogMessage": True},
        "targets": [{"datasource": {"type": "loki", "uid": loki_uid},
            "expr": f'{{host="{hostname}", critical="true", category="Memory/OOM"}}',
            "maxLines": 500,
            "queryType": "range",
            "refId": "A"}],
        "title": "ðŸ§  Memory/OOM Issues",
        "type": "logs"
    })
    panel_id += 1
    y_pos += 6
    
    # Critical Events - CPU & Kernel Panic
    panels.append({
        "datasource": {"type": "loki", "uid": loki_uid},
        "gridPos": {"h": 6, "w": 12, "x": 0, "y": y_pos},
        "id": panel_id,
        "options": {"dedupStrategy": "none", "enableLogDetails": True, "showTime": True, "sortOrder": "Ascending", "wrapLogMessage": True},
        "targets": [{"datasource": {"type": "loki", "uid": loki_uid},
            "expr": f'{{host="{hostname}", critical="true", category="CPU & Kernel Panic"}}',
            "maxLines": 500,
            "queryType": "range",
            "refId": "A"}],
        "title": "âš¡ CPU & Kernel Panic",
        "type": "logs"
    })
    panel_id += 1
    
    # Critical Events - Security & Antivirus
    panels.append({
        "datasource": {"type": "loki", "uid": loki_uid},
        "gridPos": {"h": 6, "w": 12, "x": 12, "y": y_pos},
        "id": panel_id,
        "options": {"dedupStrategy": "none", "enableLogDetails": True, "showTime": True, "sortOrder": "Ascending", "wrapLogMessage": True},
        "targets": [{"datasource": {"type": "loki", "uid": loki_uid},
            "expr": f'{{host="{hostname}", critical="true", category="Security & Antivirus"}}',
            "maxLines": 500,
            "queryType": "range",
            "refId": "A"}],
        "title": "ðŸ”’ Security & Antivirus",
        "type": "logs"
    })
    panel_id += 1
    y_pos += 6
    
    # Critical Events - Network Issues
    panels.append({
        "datasource": {"type": "loki", "uid": loki_uid},
        "gridPos": {"h": 6, "w": 24, "x": 0, "y": y_pos},
        "id": panel_id,
        "options": {"dedupStrategy": "none", "enableLogDetails": True, "showTime": True, "sortOrder": "Ascending", "wrapLogMessage": True},
        "targets": [{"datasource": {"type": "loki", "uid": loki_uid},
            "expr": f'{{host="{hostname}", critical="true", category="Network Issues"}}',
            "maxLines": 500,
            "queryType": "range",
            "refId": "A"}],
...
def main():
    st.markdown('<h1 class="main-header">ðŸ“Š SOSreport Analyzer V2</h1>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    if uploaded_file is not None:
        ...
        if st.button("ðŸš€ Process SOSreport", type="primary", use_container_width=True):
            ...
            try:
                ...
                status_text.text("ðŸ” Analyzing for peaks and anomalies...")
                ...
                st.subheader("ðŸ–¥ï¸ Basic System Information")
                ...
                if critical_events:
                    ...
                    # Display by category
                    crit_cols = st.columns(len(LOG_PATTERNS))
                    for idx, (category, count) in enumerate(critical_summary.items()):
                        with crit_cols[idx]:
                            icon = {
                                "File System & Disk": "ðŸ’¾",
                                "Memory/OOM": "ðŸ§ ",
                    ...
                    # Show sample critical events
                    with st.expander("ðŸ” View Critical Events Sample (first 50)"):
                        for i, event in enumerate(critical_events[:50]):
                            ts = event.get('timestamp', 'N/A')
                            ts_str = ts.strftime('%Y-%m-%d %H:%M:%S') if ts else 'N/A'
                            cat = event.get('category', '')
                            pattern = event.get('pattern', '')
                            msg = event.get('message', '')[:200]
                            st.text(f"[{ts_str}] [{cat}] [{pattern}] {msg}")
                else:
                    st.markdown("---")
...
'''
                ...
                if push_logs and logs:
                    ...
                    # Push critical events to Loki with category labels
                    if critical_events:
                        status_text.text("ðŸš¨ Pushing critical events to Loki...")
                        pushed_critical, critical_error = push_critical_events_to_loki(critical_events, hostname)
                        results['critical_events'] = pushed_critical
                        if pushed_critical > 0:
                            st.success(f"ðŸš¨ Pushed {pushed_critical:,} critical events to Loki")
                        if critical_error:
                            st.warning(f"âš ï¸ Critical events errors: {critical_error}")
    ...
    st.markdown(
        ...
        "SOSreport Analyzer V2 | Powered by Streamlit, InfluxDB, Loki & Grafana | Critical Events Detection"
        "</div>",
        unsafe_allow_html=True
    )
